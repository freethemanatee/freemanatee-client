...
public class AutoOffhandCrystal extends Module
{
    ...
    public void onUpdate() {
        ...
        this.crystals = AutoOffhandCrystal.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.GOLDEN_APPLE).mapToInt(ItemStack::getCount).sum();
        if (AutoOffhandCrystal.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE) {
            ++this.crystals;
        }
        else {
            ...
            if (AutoOffhandCrystal.mc.player.inventory.itemStack.isEmpty()) {
                ...
                int t = -1;
                for (int i = 0; i < 45; ++i) {
                    if (AutoOffhandCrystal.mc.player.inventory.getStackInSlot(i).getItem() == Items.GOLDEN_APPLE) {
                        t = i;
                        break;
                    }
                }
