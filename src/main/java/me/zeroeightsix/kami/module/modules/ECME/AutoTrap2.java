...
public class AutoTrap2 extends Module {
    ...
    protected void onEnable() {
        if (mc.player == null) {
            ...
            if ((Boolean)this.announceUsage.getValue()) {
                Command.sendChatMessage("we trappin");
            }
    ...
    protected void onDisable() {
        if (mc.player != null) {
            ...
            this.lastHotbarSlot = -1;
            if ((Boolean)this.announceUsage.getValue()) {
                Command.sendChatMessage("we aint trappin no more");
            }
    ...
    public void onUpdate() {
        if (mc.player != null) {
            if ((Boolean)this.chad.getValue() && ((double)Math.round(mc.player.posY) != this.yHeight || (double)Math.round(mc.player.posX) != this.xPos || (double)Math.round(mc.player.posY) != this.zPos)) {
                this.disable();
            }
