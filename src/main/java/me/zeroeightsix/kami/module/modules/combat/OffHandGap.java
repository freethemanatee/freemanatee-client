...
public class OffHandGap extends Module {
    ...
    private boolean shouldTotem() {
        if (mc.player != null) {
            return (mc.player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() == Items.ELYTRA || !nearPlayers() || mc.player.getHealth() + mc.player.getAbsorptionAmount() <= health.getValue() || (crystalCheck.getValue() && !isGapplesAABBEmpty()));
        }
        return (mc.player.getHealth() + mc.player.getAbsorptionAmount()) <= health.getValue() || mc.player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() == Items.DIAMOND_CHESTPLATE || !nearPlayers() || (crystalCheck.getValue() && !isGapplesAABBEmpty());
    }
